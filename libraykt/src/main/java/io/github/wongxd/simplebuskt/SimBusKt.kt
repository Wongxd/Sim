 ...
 * 事件管理类
 */
class SimBusKt private constructor() {

    companion object {

        private val default: SimBusKt = SimBusKt()

        fun getInstance(): SimBusKt = default
     ...
     * @param observer
     */
    fun register(code: Int = -1, event: Class<*> = DefaultEvent::class.java, observer: IObserver) {
        observable.addObserver(code, event, observer)
    }
     ...
     * @param observer
     */
    fun unregister(code: Int = -1, event: Class<*> = DefaultEvent::class.java, observer: IObserver) {
        observable.removeObserver(code, event, observer)
    }
     ...
     * @param event 事件对象
     */
    fun notifyObservers(code: Int = -1, event: Any = DefaultEvent()) {
    ...
    private fun addSubscriber(simEventMethod: SimEventMethod) {
        ...
        val observer = object : IObserver {
            override fun onMessageReceived(event: Any, code: Int) {
                if (code == sCode)
                    postToThread(event, simEventMethod)
            }
        }
        ...
        register(sCode, eventType, observer)
...
//        Log.e(TAG, event.toString() + "---" + simEventMethod.code + "---" + simEventMethod.eventType + "---" + simEventMethod.threadMode)
    ...
    fun unregisterClass(subscriber: Any) {
        ...
        simEventMethods?.forEach { unSubscribeMethod(subscriber, it) }
    ...
    private fun unSubscribeMethod(subscriber: Any, simEventMethod: SimEventMethod) {
        val eventType = simEventMethod.eventType
        val simEventMethods = eventSimEventMethods[eventType] ?: return
    ...
    private fun unSubscribeIObserver(subscriber: Any, simEventMethod: SimEventMethod) {
        val observers = subscriberIObservers[subscriber] ?: return
        val code = simEventMethod.code
        val event = simEventMethod.eventType
        observers.forEach { unregister(code, event, it) }
    }
